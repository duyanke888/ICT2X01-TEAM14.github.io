...
CycleX是一种利用虚拟现实（VR）技术的交互式应用程序，旨在将安全的自行车骑行法则灌输给不一定了解自行车规则和道德准则的自行车手和非自行车手。 CycleX希望通过允许用户经历沉浸式虚拟体验并从骑自行车者和/或行人的角度通过一系列场景决定他们的行动来实现这一目标。
...
If the user selects "Play", they will be directed to the vehicle page where they are allowed to choose their mode of transportation.d
```
